...
void pca9685::set_pwm_off(int pwm, int off){
    printf("pca9685::set_pwm_off -> NOT IMPLEMENTED YET\n");
}

static int min = 1000, max = 1500;

void pca9685::change_servo_pwm_limits(int _min, int _max){
    min = _min;
    max = _max;
}

void pca9685::set_servo_motor(int pin, double per){
    int val = (int) ((max - min) * per + min);
    pca9685::set_pwm_ms(pin, val);
}
